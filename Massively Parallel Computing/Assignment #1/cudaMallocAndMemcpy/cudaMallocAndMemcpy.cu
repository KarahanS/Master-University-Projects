...
int main(int argc, char** argv)
{
    ...
    const size_t dimA = 8;
    reinterpret_cast<float*>(cudaMallocHost(&h_a, dimA * sizeof(float))); // pinned memory
    ...
    cudaMemcpy(d_a, h_a, memSize, cudaMemcpyHostToDevice);  // copy h_a to d_a

    // Part 3 of 5: device to device memory copy
    cudaMemcpy(d_b, d_a, memSize, cudaMemcpyDeviceToDevice);  // copy d_a to d_b
    ...
    cudaMemcpy(h_a, d_b, memSize, cudaMemcpyDeviceToHost);  // copy d_b to h_a
